 ...
 *
 * ⚠️ **어느 이름이 어느 작업인지는 여기 적지 않는다.** 이 저장소는 공개이고,
 * 이름↔작업 매핑을 적는 순간 이름을 가린 의미가 사라진다.
 * 매핑 정본은 비공개 harness 의 `contexts/personal/blog/observatory-names.json` 이고,
 * 새니타이저가 해시 id 로 바꿔 스냅샷에 넣는다. 여기 배열은 **목업 표시용**이라 순서에 뜻이 없다.
 */
const DRAFT_NAMES = [
  "관측탑자리",
  "대장간자리",
